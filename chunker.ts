...
class BibtexParser {
  ...
  parse(string, max_entries): Chunk[] {
    this._progress = 0
    this.pos = 0
    this.input = string
  
    this.parsing = null
    return this.bibtex(max_entries)
  }
  ...
  progress() {
    const progress = Math.round((this.pos / this.input.length * 100) / 5) * 5
  ...
  bibtex(max_entries): Chunk[] {
    ...
    while (this.pos < this.input.length) {
      ...
      try {
        ...
        switch (d) {
          ...
          default:
            ...
            chunk.entry = true
            entries++
            if (max_entries && entries >= max_entries) return chunks
            break
        }
      ...
      if (chunk.error && chunks.length && chunks[last].error) {
...
//Runs the parser
export function parse(input, max_entries = 0) {
  return (new BibtexParser).parse(input, max_entries)
}
