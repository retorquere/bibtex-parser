  ...
  bibtex(chunks) {
    while (this.pos < this.input.length) {
      ...
      try {
        ...
        error = err.message,
        this.pos = offset.pos + 1
        while (this.pos < this.input.length && this.input[this.pos] != '@') this.pos++
      }

      const text = this.input.substring(offset.pos, this.pos)

      const last = chunks.length - 1
      if (error && chunks.length && chunks[last].error) {
        chunks[last].text += text
      } else {
        chunks.push({ text, offset, error })
      }
    }
  }
}
...
export function parse(input) {
  const chunks: Array<{ text: string, error?: string, offset: { pos: number, line: number } }> = []
  new BibtexParser(input, chunks)
  return chunks
}
